...
public:
    ...
    explicit pector(allocator_type const& alloc): // added explicit
        _storage(alloc)
    { }
    ...
    explicit pector(size_type n) // missing
    {
        _storage.force_allocate(n);
        _storage.force_size(n);
        _storage.construct(_storage.begin(), _storage.end());
    }
...
// template <class Iter, class Alloc = std::allocator<typename std::iterator_traits<Iter>::value_type>, class SizeType = size_t, class RecommendedSize = default_recommended_size, bool check_size_overflow = true>
// pector ( Iter b, Iter e, Alloc = Alloc(), SizeType= SizeType(), RecommendedSize = RecommendedSize (), bool )->pector<typename std::iterator_traits<Iter>::value_type, Alloc, SizeType, RecommendedSize, check_size_overflow>;
